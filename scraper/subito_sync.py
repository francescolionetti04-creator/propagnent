"""
HouseRadar — Subito.it Sync
============================
Scraping Subito.it (curl_cffi + Chrome120 impersonation) + push a Render
via POST /api/sync.

Sprint 5.3: l'IP cloud di Render è blacklistato Akamai (HTTP 403). L'IP del
VPS Hetzner (residenziale-like) invece risponde 200 con il warm-up cookie
sequence. Quindi lo scraping vive sul VPS e i risultati vengono pushati a
Render via /api/sync con SYNC_TOKEN.

Strategia DB:
  - Su GitHub Actions (GITHUB_ACTIONS=true): /tmp/houseradar_subito.db
  - In locale/VPS: backend/propagnent.db se esiste, altrimenti /tmp

Env vars: SYNC_TOKEN, HOUSERADAR_URL (default https://houseradar.onrender.com)
"""
...
_TMP_DB   = "/tmp/houseradar_subito.db"
...
def main():
    ...
    try:
        n = subito_api.scrapa_subito()
        print(f"[Sync] Scraping completato — {n} nuovi annunci nel DB locale\n")
    except Exception as e:
        print(f"[Sync] Errore scraping {LABEL}: {e}")

    annunci = leggi_annunci()
    print(f"[Sync] Annunci {LABEL} nel DB locale: {len(annunci)}")
    if not annunci:
        print("[Sync] Nessun annuncio — fine.")
        return

    print(f"[Sync] Fase 2 — Invio a {HOUSERADAR_URL}/api/sync ...")
    try:
